...
<script>
  ...
  <div class="d-grid gap-2"><button class="btn btn-primary btn-lg" id="btn-start-battle"><i class="bi bi-play-fill"></i> Mulai Bertarung (${BATTLE_COST}💰)</button></div>
  `);
  ...
  <div class="row">${state.userHeroes.map(h=>`<div class="col-12 mb-3"><div class="card ${h.id===state.selectedHeroId?'border-primary':''}"><div class="card-body d-flex align-items-center"><span style="font-size:48px;margin-right:16px;">${h.emoji||'👤'}</span><div class="flex-grow-1"><div class="d-flex justify-content-between"><h5 class="card-title">${h.name} Lv.${h.level}</h5>${h.id===state.selectedHeroId?'<span class="badge bg-primary">✔️️</span>':''}</div><div class="row small mb-1"><div class="col-6">❤️ HP: ${h.stats.hp}</div><div class="col-6">⚔️ ATK: ${h.stats.atk}</div><div class="col-6">🛡️ DEF: ${h.stats.def}</div><div class="col-6">⏱️ ASPD: ${h.stats.aspd}s</div></div><div class="row small text-muted mb-2"><div class="col-6">🎯 Akurasi: ${Math.round(h.stats.accuracy * 100)}%</div><div class="col-6">👟 Evasi: ${Math.round(h.stats.evasion * 100)}%</div><div class="col-6">🛡️ Block: ${Math.round(h.stats.block_chance * 100)}%</div><div class="col-6">⚡ Crit: +${Math.round((h.stats.crit_chance_bonus || 0) * 100)}%</div><div class="col-6">🔄 Counter: +${Math.round((h.stats.counter_chance_bonus || 0) * 100)}%</div></div><button class="btn btn-sm ${h.id===state.selectedHeroId?'btn-disabled':'btn-outline-primary'} w-100 mt-2 select-hero-btn" data-hero-id="${h.id}" ${h.id===state.selectedHeroId?'disabled':''}>${h.id===state.selectedHeroId?'Terpilih':'Pilih Hero Ini'}</button></div></div></div></div>`).join('')}</div>
  ...
  <h5>⛏️ Mining Gold</h5><p class="small">Gold dapat ditambang otomatis setiap jam. Klik ikon 💰 di atas untuk klaim.</p>
