...
class BattleSimulator
{
  ...
  public function runSimulation(): array
  {
    ...
    while ($pHp > 0 && $eHp > 0) {
      ...
      if ($pTimer >= $this->player['aspd']) {
        $pTimer = 0.0;
        $damageData = $this->calculateDamage($this->player, $this->enemy, $eHp);
      ...
      if ($eTimer >= $this->enemy['aspd']) {
        $eTimer = 0.0;
        $damageData = $this->calculateDamage($this->enemy, $this->player, $pHp);
  ...
  * @param float $defenderCurrentHp (tidak digunakan, tapi bisa untuk efek tertentu)
  * @return array ['value' => int, 'blocked' => bool, 'message' => string]
  */
  protected function calculateDamage(array $attacker, array $defender, float $defenderCurrentHp): array
  {
